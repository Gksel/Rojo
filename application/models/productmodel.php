...
class ProductModel extends CI_Model{
	...
	function getproductinfo($product_id){
		$sql="
				...
				, c.user_realname, c.user_number
				, d.step1_user_number, d.step1_date_cloth_due, d.step1_date_cloth_submitted, d.step1_status_num, d.step1_approve_status, d.step1_approve_time_1, d.step1_approve_time_2, d.step1_approve_time_3, d.step1_approve_time_4
				...
				, g.step4_approve_status, g.step4_approve_time, g.step4_status_num, g.step4_approve_time_1, g.step4_approve_time_2
				, h.step5_approve_status, h.step5_approve_time
				, t.factory_name_en, t.factory_name_ch
				
				FROM ".DB_PRE()."product_list AS a 
						
				LEFT JOIN ".DB_PRE()."product_step1 AS d ON a.product_id = d.product_id
				LEFT JOIN ".DB_PRE()."product_step2 AS e ON a.product_id = e.product_id
				LEFT JOIN ".DB_PRE()."product_step3 AS f ON a.product_id = f.product_id
				LEFT JOIN ".DB_PRE()."product_step4 AS g ON a.product_id = g.product_id AND isdel = 0
				LEFT JOIN ".DB_PRE()."product_step5 AS h ON a.product_id = h.product_id
						
				LEFT JOIN ".DB_PRE()."user_list AS c ON d.uid = c.uid
				LEFT JOIN ".DB_PRE()."system_product_factory AS t ON d.factory_id = t.factory_id
	...
	function getproductlist($con=array(),$iscount=0){
		...
		$limit="";
		$leftjoin = "";
		if(isset($con['category_id'])){$leftjoin .= " LEFT JOIN ".DB_PRE()."product_category AS e ON a.product_id = e.product_id";}
		...
		if(isset($con['keyword'])){if($where!=""){$where .=" AND";}else{$where .=" WHERE";} $where .= " ((a.product_name_en LIKE '%".addslashes($con['keyword'])."%') OR (a.product_name_ch LIKE '%".addslashes($con['keyword'])."%') OR (a.product_SKUno LIKE '%".addslashes($con['keyword'])."%')) ";}
		...
		if($iscount==0){
			$sql="
				...
				LEFT JOIN ".DB_PRE()."system_product_brand AS b ON a.brand_id = b.brand_id
	...
	function getproductcategorylist($con=array(),$iscount=0){
		...
		if($iscount==0){
			$sql="SELECT a.* FROM ".DB_PRE()."system_product_category a $where $order_by $limit";
			$result=$this->db->query($sql)->result_array();
			if(!empty($result)){
				return $result;
			}else{
			...
			$sql="SELECT count(*) as count FROM ".DB_PRE()."system_product_category a $where $order_by";
			$result=$this->db->query($sql)->row_array();
			if(!empty($result)){
				return $result['count'];
			}else{
	...
	function getproductcategoryinfo($category_id){
		$sql="select * from ".DB_PRE()."system_product_category where category_id=".$category_id;
		$query=$this->db->query($sql);
		if($query->num_rows()>0){
			return $query->row_array();
		}else{
	...
	function getproductcategoryinfo_BYshorturl($shorturl){
		$sql="select * from ".DB_PRE()."system_product_category where shorturl = '".$shorturl."'";
		$query=$this->db->query($sql);
		if($query->num_rows()>0){
			return $query->row_array();
		}else{
	...
	function add_productcategory($arr){
		$this->db->insert(DB_PRE().'system_product_category',$arr);
		return $this->db->insert_id();
	}
	//修改商品分类
	function edit_productcategory($category_id,$arr){
		$this->db->update(DB_PRE().'system_product_category',$arr,array('category_id'=>$category_id));
	}
	//删除商品分类
	function del_productcategory($category_id){
		$this->db->delete(DB_PRE().'system_product_category', array('category_id'=>$category_id));
	}
	//查询产品品牌列表
