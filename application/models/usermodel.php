...
class UserModel extends CI_Model{
	...
	function getuserlist($con=array(),$iscount=0){
		...
		if(isset($con['keyword'])){if($where!=""){$where .=" AND";}else{$where .=" WHERE";} $where .= " ((u.user_phone LIKE '%".addslashes($con['keyword'])."%') OR (u.user_realname LIKE '%".addslashes($con['keyword'])."%')) ";}
	...
	//查询用户地址列表
	function getuser_addresslist($con=array(),$iscount=0){
		$where="";
		$order_by="";
		$limit="";
		if(isset($con['other_con'])){if($where!=""){$where .=" AND";}else{$where .=" WHERE";} $where .= " ".$con['other_con'];}
	...
	//用户地址详细
	function getuser_addressinfo($address_id){
