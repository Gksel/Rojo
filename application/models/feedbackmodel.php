...
class FeedbackModel extends CI_Model{
	...
	function del_feedback($feedback_id){
		$this->db->delete(DB_PRE().'feedback_list', array('feedback_id'=>$feedback_id));
	}
	...
	function getfeedbacklist($con=array(),$iscount=0){
		...
		if(isset($con['feedback_id'])){if($where!=""){$where .=" AND";}else{$where .=" WHERE";} $where .= " a.feedback_id =".$con['feedback_id'];}
		...
		if($iscount==0){
			$sql="
					...
					b.user_realname AS feedback_user_realname, b.user_phone AS feedback_user_phone, b.user_type AS feedback_user_type
