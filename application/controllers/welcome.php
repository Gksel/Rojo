...
// 		for ($i = 1; $i <= 2000; $i++) {
// 			$arr = array('product_key'=>randkey(32), 'created'=>mktime(), 'edited'=>mktime());
// 			//商品信息
// 			$arr['product_name_en'] = $i;
// 			$arr['product_name_ch'] = $i;
...
// 		}
	...
	function shop(){
		...
		$this->load->view('default/shop_list');
	}
	function company(){
		$this->session->set_userdata('menu', 'company');
		$this->load->view('default/company_list');
	}
	function architect(){
		$this->session->set_userdata('menu', 'architect');
		$this->load->view('default/architect_list');
	}
	/* 上传附件
	 * */
	function upfile(){
...
//				如果上传的图片大小和给定的图片尺寸大小一致则不操作
	...
	function calendar_select($year = 0, $month = 0) {
		...
		$prefs ['template'] = '
		   ...
		   {heading_previous_cell}<th height="50" colspan="2" style="padding:0px;-moz-border-top-left-radius: 4px;-webkit-border-top-left-radius: 4px;"><a href="javascript:;" onclick="togetshiwucalendar_month(\'' . $id . '\',{previous_url})">上一月</a></th>{/heading_previous_cell}
		   {heading_title_cell}<th colspan="3" style="font-size:14px;padding:0px;">{heading}</th>{/heading_title_cell}
		   {heading_next_cell}<th colspan="2" style="padding:0px;-moz-border-top-right-radius: 4px;-webkit-border-top-right-radius: 4px;"><a href="javascript:;" onclick="togetshiwucalendar_month(\'' . $id . '\',{next_url})">下一月</a></th>{/heading_next_cell}
		   ...
		   {cal_cell_start}<td style="width:' . ((1 / 7) * 100) . '%;padding:0px;"><div style="float:left;text-align:center;width:90%;height:25px;line-height:25px;margin-left:3.5%;margin-top:5px;border:1px solid #EFEFEF;">{/cal_cell_start}
	
		   {cal_cell_content}<a href="{content}" id="shiwu_{day_id}" onclick="togetrilidatatoinput(\'' . $id . '\',{showthings})"><font style="float:left;width:100%;height:25px;line-height:25px;text-align:center;background:#efefef;color:red;font-weight:bold;-moz-border-radius: 3px;-webkit-border-radius: 3px;">{day}</font></a>{/cal_cell_content}
		   {cal_cell_content_today}<a href="{content}" id="shiwu_{day_id}" onclick="togetrilidatatoinput(\'' . $id . '\',{showthings})"><font style="float:left;width:100%;height:25px;line-height:25px;text-align:center;background:#efefef;color:red;font-weight:bold;-moz-border-radius: 3px;-webkit-border-radius: 3px;">{day}</font></a>{/cal_cell_content_today}
	
		   {cal_cell_no_content}<a href="javascript:;" id="shiwu_{day_id}" onclick="togetrilidatatoinput(\'' . $id . '\',{showthings})"><font style="float:left;width:100%;height:25px;line-height:25px;text-align:center;background:#efefef;color:#1F9237;font-weight:bold;-moz-border-radius: 3px;-webkit-border-radius: 3px;">{day}</font></a>{/cal_cell_no_content}
		   {cal_cell_no_content_today}<a href="javascript:;" id="shiwu_{day_id}" onclick="togetrilidatatoinput(\'' . $id . '\',{showthings})"><font style="float:left;width:100%;height:25px;line-height:25px;text-align:center;background:#efefef;color:#1F9237;font-weight:bold;-moz-border-radius: 3px;-webkit-border-radius: 3px;">{day}</font></a>{/cal_cell_no_content_today}
	...
	function tobookyourtailor(){
		...
		$arr = array('shijian_date'=>$currentshiwudi, 'uid'=>$uid, 'shijianduan_id'=>$current_shijianduan_id, 'shijianduan_num'=>$current_shijianduan_num, 'status'=>0, 'created'=>mktime(), 'edited'=>mktime());
