...
class Appointment extends CI_Controller{
	...
	function add_appointment(){
		...
		if($appointment_price_promotion == ''){$appointment_price_promotion = 0;}
		
		$appointment_netweight = $this->input->post('appointment_netweight');//净重
		if($appointment_netweight == ''){$appointment_netweight = 0;}
		$appointment_size = $this->input->post('appointment_size');//规格
		if($appointment_size == ''){$appointment_size = 0;}
	...
	function appointmentsetting_isclose($id){
		...
		if($isclose == 1){
			$this->db->insert(DB_PRE().'appointmentsetting_close', array('shijian_date'=>$id_splid[0], 'shijianduan_id'=>$id_splid[1], 'shijianduan_num'=>$id_splid[2], 'created'=>mktime(), 'edited'=>mktime()));
		}else{
