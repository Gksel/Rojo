			...
			<tr>
				...
				<td>
					<?php if($pointsettinglist[$i]['pointsetting_id'] == 1){?>
							&yen;100 = <?php echo ($pointsettinglist[$i]['pointsetting_value'] * 100);?>
					...
					<?php }else if($pointsettinglist[$i]['pointsetting_id'] == 3){?>
							100 <?php if($this->langtype == '_ch'){echo '积分';}else {echo 'Points';}?> = <?php echo '&yen;'.($pointsettinglist[$i]['pointsetting_value'] * 100);?>
					<?php }?>
				...
				<td>
					...
					<div style="float:left;width:100%;margin-top:5px;">
						<?php 
							if($this->langtype == '_ch'){$history_text = '历史记录';}else {$history_text = 'History';}
							...
							if($count > 0){
								...
								$text = $history_text.' <font style="color:#000;font-weight:bold;">('.$count.')</font>';
							}else{
								$alink = base_url().'index.php/admins/user/pointsetting_historylist/'.$pointsettinglist[$i]['pointsetting_id'].'?backurl='.$current_url_encode;
								$text = $history_text.' <font style="color:#000;font-weight:bold;">('.$count.')</font>';
							}
