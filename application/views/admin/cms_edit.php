...
<form id="form_cms_save" method="post">
	<table class="gksel_normal_tabpost">
		<?php for ($lc = 0; $lc < count($lancodelist); $lc++) {?>
			...
			?>
			<tr>
				<td align="right" width="150"><?php echo lang('cy_name')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
				<td align="left">
					<input type="text" name="cms_name<?php echo $lancodelist[$lc]['langtype']?>" style="width:300px;" value="<?php echo $cmsinfo['cms_name'.$lancodelist[$lc]['langtype']]?>"/>
					<div class="tipsgroupbox"><div class="request">*</div></div>
				</td>
			</tr>
			<tr>
				<td align="right"><?php echo lang('cy_description')?> <?php if(count($lancodelist) != 1){echo '('.$lancodelist[$lc]['langname'].')';}?></td>
			    <td align="left">
			    	<div style="float:left;width:800px;">
			    		<textarea id="cms_description<?php echo $lancodelist[$lc]['langtype']?>" name="cms_description<?php echo $lancodelist[$lc]['langtype']?>"><?php echo debaseurlcontent($cmsinfo['cms_description'.$lancodelist[$lc]['langtype']])?></textarea>
			    	</div>
		...
		<tr>
			<td>
				<input name="cms_id" type="hidden" value="<?php echo $cmsinfo['cms_id']?>"/>
				<input name="backurl" type="hidden" value="<?php echo $backurl;?>"/>
			</td>
			<td align="left">
				<div class="gksel_btn_action_on" onclick="tosave_cmsinfo()"><?php echo lang('cy_save')?></div>
			</td>
		</tr>
	</table>
</form>
