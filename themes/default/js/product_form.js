	...
	//用户FORM---添加
	function product_forminfo(){
		var lancodelist = getlancodelist();
		if(isajaxsaveing == 0){
			//具体点击的按钮
		
			actionsubmit_button = $('div[onclick="toadd_forminfo()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
...
//			var subbackurl = $('input[name="subbackurl"]').val();
			...
			//----定义的字段------START
				...
				GETOBJ[GETOBJ_num]['field_name'] = 'jacket_lengthbody';
				GETOBJ[GETOBJ_num]['field_realname'] = 'jacket_lengthbody';
				GETOBJ[GETOBJ_num]['field_type'] = "input";
				GETOBJ[GETOBJ_num]['field_CMNAME'] = 'Jacket 夹克';
			//----定义的字段------END
...
//			postOBJ.subbackurl = subbackurl;
			...
			if(returnOBJ.ispass == 1){
				$.post(baseurl+'index.php/product/tosubmitproduct_step2/', postOBJ, function (data){
					var obj = eval( "(" + data + ")" );
