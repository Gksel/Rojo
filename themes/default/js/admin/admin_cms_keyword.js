	...
	//关键字信息---保存
	function tosave_keywordinfo(keyword_id){
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_keywordinfo('+keyword_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			if(ispass == 1){
				$.post(baseurl+'index.php/admins/cms/edit_keyword/'+keyword_id, {
					//返回url
					backurl: backurl,
					//关键字信息
					keyword_name_en: keyword_name_en,
