    ...
    */  
	...
	function todel_feedback(id, name){
		var title = '您确定要删除此条意见反馈吗？';
		var subtitle = '';
		del_url = encodeURI(baseurl+"index.php/member/del_feedback/"+id);
...
/***************************FeedBakc*********************************/
	...
	function toadd_feedback(){
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="toadd_feedback()"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			var ispass=1;
			if(isNull.test(feedback_content)){ispass = 0;$('textarea[name="feedback_content"]').next().append(ajax_returnrequiredorerror_BOX('意见反馈不得为空'));}else{$('textarea[name="feedback_content"]').next().find('.requestbox').remove();}
			
			if(ispass == 1){
				$.post(baseurl+'index.php/member/add_feedback', {
					//返回url
					backurl: backurl,
	...
	//保存意见反馈
	function tosave_feedback(feedback_id){
		if(isajaxsaveing == 0){
			//具体点击的按钮
			actionsubmit_button = $('div[onclick="tosave_feedback('+feedback_id+')"]');
			//ajax正在保存中
			isajaxsaveing = 1;
			//返回url
			var backurl = $('input[name="backurl"]').val();
			...
			if(ispass == 1){
				$.post(baseurl+'index.php/member/edit_feedback/'+feedback_id, {
					//返回url
					backurl: backurl,
					feedback_content: feedback_content,
				},function (data){
...
//						isajaxsaveing = 0;//ajax正在保存中 --- 释放
	...
	function tounbindaccount(){
		$('.gksel_unbindaccount_box_bg, .gksel_unbindaccount_box').fadeIn(800);
	}
	//取消绑定 --- 处理方法
	function unbindaccount(){
		if(del_loading == 0){
			del_loading = 1;
			$('.gksel_unbindaccount_content .title').html('&nbsp;');
			$('.gksel_unbindaccount_content .subtitle').html('<div style="float:left;width:100%;text-align:center;"><img src="'+baseurl+'themes/default/images/indicator.gif"/></div>');
			$.post(baseurl+'index.php/member/unbindaccount',function (data){
				location.href = currenturl;
			});
		}
	}
	//取消绑定 --- 关闭
	function unbindaccount_close(){
		$('.gksel_bindaccount_box_bg, .gksel_bindaccount_box').fadeOut(800);
	}
	
	//绑定微信
	function tobindaccount(){
		$.post(baseurl+'index.php/member/tobindaccount', function (data){
			$('.gksel_bindaccount_content').find('.qrcodearea').html(data);
			$('.gksel_bindaccount_box_bg, .gksel_bindaccount_box').fadeIn(800);
		})
	}
	//绑定微信 --- 处理方法
	function bindaccount(){
		if(del_loading == 0){
			del_loading = 1;
			$('.gksel_bindaccount_content .title').html('&nbsp;');
			$('.gksel_bindaccount_content .subtitle').html('<div style="float:left;width:100%;text-align:center;"><img src="'+baseurl+'themes/default/images/indicator.gif"/></div>');
			$.post(baseurl+'index.php/member/bindaccount',function (data){
				location.href = currenturl;
			});
		}
	}
	//绑定微信 --- 关闭
	function bindaccount_close(){
		$('.gksel_bindaccount_box_bg, .gksel_bindaccount_box').fadeOut(800);
	}